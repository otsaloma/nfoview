...
timeout 5 bin/nfoview README.md 2>&1`. Exit 124 is expected on timeout.
Use `pytest -s` to expose GTK/GLib warnings in tests, and
`G_DEBUG=fatal-warnings` to stop on warnings when debugging.

Standalone scripts must import nfoview from this checkout, not an
installed copy. Set `PYTHONPATH` or `sys.path` accordingly; verify
`nfoview.__file__` if unsure.
